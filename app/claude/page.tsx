...
import { workoutData } from "@/lib/workout-data-claude"
import { ThemeToggle } from "@/components/theme-toggle"
import type { ExerciseProgress } from "@/lib/types"

const STORAGE_KEY = "exerciseProgress-claude"

export default function WorkoutTracker() {
  const [currentDay, setCurrentDay] = useState<number>(0)
  const [exerciseProgress, setExerciseProgress] = useState<ExerciseProgress>({})

  // Get current day of week (1-4 -> Wed, Thu, Fri, Sat)
  useEffect(() => {
    const today = new Date().getDay()
    // Map: Wed(3)->1, Thu(4)->2, Fri(5)->3, Sat(6)->4. Otros días -> 1 (próxima sesión)
    const dayMap: Record<number, number> = { 3: 1, 4: 2, 5: 3, 6: 4, 0: 1, 1: 1, 2: 1 }
  ...
  useEffect(() => {
    if (Object.keys(exerciseProgress).length > 0) {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(exerciseProgress))
    }
  }, [exerciseProgress])

  const handlePrevDay = () => {
    setCurrentDay((prev) => (prev > 1 ? prev - 1 : 4))
  }

  const handleNextDay = () => {
    setCurrentDay((prev) => (prev < 4 ? prev + 1 : 1))
  }
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8">
      <header className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight mb-2">Rutina CLAUDE</h1>
          ...
          >
            <div className="text-center">
              <div className="text-xs text-muted-foreground">
                {["MIÉRCOLES", "JUEVES", "VIERNES", "SÁBADO"][day - 1]}
              </div>
